...
import { loadPrefs, savePrefs, DEFAULT_PREFS, scheduleAll, requestPermission } from '../services/notifications.js';
...
const NotificationSettings = ({ medications, attacks }) => {
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement('div', { className: 'space-y-4' },
        React.createElement('div', { className: 'flex items-center justify-between' },
          React.createElement(Label, null, 'Enable reminder'),
          React.createElement(Toggle, {
            enabled: prefs.preventativeEnabled,
            onChange: (val) => updatePref('preventativeEnabled', val),
          })
        ),

        prefs.preventativeEnabled && React.createElement(
          React.Fragment,
          null,
          ...
          React.createElement('p', { className: 'text-sm text-dark-text-secondary italic' },
            "You'll be notified every morning and evening until you confirm"
          ),
    ...
    // ── Daily Trigger Reminder ──
    React.createElement(
      Card,
      { title: '📋 Daily Trigger Reminder' },
      React.createElement('div', { className: 'space-y-4' },
        React.createElement('div', { className: 'flex items-center justify-between' },
          React.createElement(Label, null, 'Enable daily trigger reminder'),
          React.createElement(Toggle, {
            enabled: prefs.triggerReminderEnabled,
            onChange: (val) => updatePref('triggerReminderEnabled', val),
          })
        ),

        prefs.triggerReminderEnabled && React.createElement(
          React.Fragment,
          null,
          React.createElement('div', null,
            React.createElement(Label, { htmlFor: 'trigger-time' }, 'Reminder time'),
            React.createElement(Input, {
              id: 'trigger-time',
              type: 'time',
              value: prefs.triggerReminderTime || '20:00',
              onChange: (e) => updatePref('triggerReminderTime', e.target.value),
            })
          ),

          React.createElement('div', null,
            React.createElement(Label, { htmlFor: 'trigger-message' }, 'Reminder message'),
            React.createElement(Input, {
              id: 'trigger-message',
              type: 'text',
              value: prefs.triggerReminderMessage || '',
              onChange: (e) => updatePref('triggerReminderMessage', e.target.value),
              placeholder: "Don't forget to log your daily triggers!",
            })
          )
        )
      )
